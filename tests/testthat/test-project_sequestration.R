...
testthat::test_that("get_orphaned_projects identifies 5 orphans in the correct sequence", {
  ...
  rc_conn <- DBI::dbConnect(RSQLite::SQLite(), dbname = ":memory:")
  purrr::walk(get_orphaned_projects_test_tables, create_a_table_from_rds_test_data, rc_conn, "get_orphaned_projects")

  rcc_billing_conn <- DBI::dbConnect(RSQLite::SQLite(), dbname = ":memory:")
  purrr::walk(c("banned_owners"), create_a_table_from_rds_test_data, rcc_billing_conn, "get_orphaned_projects/rcc_billing")

  # TODO: this needs to be dynamic
  redcapcustodian::set_script_run_time(lubridate::ymd_hms("2023-04-01 12:00:00"))

  result <- get_orphaned_projects(rc_conn, rcc_billing_conn)
