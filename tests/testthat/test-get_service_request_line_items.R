...
mock_service_requests<- readRDS(testthat::test_path("get_service_request_lines", "service_requests.rds"))|> mutate(
  irb_number = c("123"),
...
)|> dplyr::mutate_all(as.integer) |> dplyr::mutate(stringAsFactors = FALSE)
...
dbWriteTable(rcc_billing_conn, "ctsi_study_id_map", mock_ctsi_study_id_map)
dbWriteTable(rcc_billing_conn, "project_details", mock_project_details)
dbWriteTable(rcc_billing_conn, "invoice_line_item", mock_invoice_line_item)


dbWriteTable(rc_conn, "redcap_entity_project_ownership", mock_redcap_entity_project_ownership)
...
duckdb_register(conn = rc_conn, name = "redcap_projects", df = updated_project_ids)
...
redcapcustodian::set_script_run_time(lubridate::ymd_hms("2023-02-05 12:00:00", tz="EST"))
...
test_that("get_service_request_line_items returns correct results", {
  result <- get_service_request_line_items(mock_service_requests, rcc_billing_conn, rc_conn)
