...
# Make Service Request line items
...
combined_new_new_service_instances <-
  bind_rows(
...
new_service_instances_diff <- redcapcustodian::dataset_diff(
  source = combined_new_new_service_instances,
...
# Create invoice_line_item rows for projects ###############################################
...
# Email CSBT
email_subject <- paste("New invoice line items for REDCap Automated Billing")
attachment_object <- sendmailR::mime_part(tmp_invoice_file, new_invoice_line_items_filename)
body <- "The attached file has new invoice line items for REDCap Automated Billing. Please load these into the CSBT invoicing system."
