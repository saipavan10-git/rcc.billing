...
#' Generate a fiscal year interval
#'
#' @param csbt_label A string representing the fiscal year range, in the format YYYY-YYYY
#' @return A lubridate interval object from the start of the UF fiscal year (July 1) to the end
#' @examples
#' \dontrun{
#' fiscal_year_interval <- generate_fiscal_year_interval("2021-2022")
#' }
#' @export
generate_fiscal_year_interval <- function(csbt_label) {
  years <- stringr::str_split(csbt_label, pattern = "-", n = 2)[[1]]

  fy_start <- lubridate::ymd(paste0(years[1], "-07-01"), tz="America/New_York")
  fy_end <- lubridate::ymd(paste0(years[2], "-07-01"), tz="America/New_York")
...
#'   \item fy_interval - A lubridate interval object from the start of the UF fiscal year (July 1) to the end, generated by \code{\link{generate_fiscal_year_interval}}
#' }
#' @examples
#' \dontrun{
...
get_fiscal_years <- function() {
  ...
  fiscal_years <- tibble::tibble(
    "csbt_label" = paste0(START_YEAR:(END_YEAR - 1), "-", (START_YEAR + 1):END_YEAR),
    "fy_interval" = generate_fiscal_year_interval(.data$csbt_label)
  )
